...
public class SiteDao {
    ...
    public final String getLogFootprint(Integer id) {
        final String sql = "SELECT LOG_FOOT_PRINT FROM SITES WHERE ID = ?";
        String  fp = null;
        try {
            Connection c = dataSource.getConnection();
            PreparedStatement ps = c.prepareStatement(sql);
            ...
            fp = rs.getString("LOG_FOOT_PRINT");
            rs.close();
            ps.close();
        } catch (SQLException e) {
            e.printStackTrace();
    ...
    public final Boolean setLogFootprint(Integer id, String footPrint) {
        final String sql = "UPDATE SITES SET LOG_FOOT_PRINT = ? WHERE ID = ?";

        try {
            Connection c = dataSource.getConnection();
            PreparedStatement ps = c.prepareStatement(sql);
            ps.setString(1, footPrint);
            ps.setInt(2, id);
